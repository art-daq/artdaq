...
int main(int argc, char * argv[])
{
	...
	app_name = config_ps.get<std::string>("application_name", "Dispatcher");
	...
	artdaq::DispatcherApp di_app(rank, app_name);
	...
	if (auto_run) {
		...
		di_app.do_initialize(config_ps, timeout, timestamp);
		di_app.do_start(art::RunID(run), timeout, timestamp);
	...
	auto commander = artdaq::MakeCommanderPlugin(config_ps, di_app);
