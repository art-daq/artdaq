		...
		while (true)
		{
			...
			if (res > 0)
			{
				...
				else
				{
					...
					TRACE(TLVL_DEBUG + 3, "receiveTableUpdatesLoop_: Received a packet of %ld bytes. 1st 16 bytes: 0x%016lx%016lx", stss, ((unsigned long*)&buffer[0])[0], ((unsigned long*)&buffer[0])[1]);  // NOLINT
