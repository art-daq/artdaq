...
try
{
	...
	desc.add_options()
        ...
        ("stop,s", "Stop an existing dispatcher art job")
        ("dispatcherHost,d", bpo::value<std::string>()->default_value("localhost"), "Dispatcher host")
        ("dispatcherPort,p", bpo::value<int>()->default_value(5266), "Dispatcher port")
        ("unique_label,l", bpo::value<std::string>()->default_value("default_art_job"), "Unique label for this art job")
