...
BOOST_AUTO_TEST_CASE(Simple)
{
	...
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 1);
...
BOOST_AUTO_TEST_CASE(SingleMode)
{
	...
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	...
	TRACE_REQUIRE_EQUAL(fps.size(), 1u);
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 5);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 2);
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	...
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), ts);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 3);
	auto emptyType = artdaq::Fragment::EmptyFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), emptyType);
	fps.pop_front();
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 7);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 4);
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	fps.clear();
...
BOOST_AUTO_TEST_CASE(BufferMode)
{
	...
	auto emptyType = artdaq::Fragment::EmptyFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), emptyType);
	TRACE_REQUIRE_EQUAL(fps.front().first->size(), artdaq::detail::RawFragmentHeader::num_words());
	fps.pop_front();
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 7);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 4);
...
BOOST_AUTO_TEST_CASE(CircularBufferMode)
{
	...
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 8);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 5);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
...
BOOST_AUTO_TEST_CASE(WindowMode_Function)
{
	...
	TRACE_REQUIRE_EQUAL(fps.size(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 2);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 2);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	auto cf2 = artdaq::ContainerFragment(*fps.front().first);
	...
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 3);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 4);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	auto cf3 = artdaq::ContainerFragment(*fps.front().first);
	...
	TRACE_REQUIRE_EQUAL(fps.size(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 4);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 5);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	auto cf4 = artdaq::ContainerFragment(*fps.front().first);
	...
	TRACE_REQUIRE_EQUAL(fps.size(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 13);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 7);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	...
	TRACE_REQUIRE_EQUAL(fps.size(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 12);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 6);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
...
BOOST_AUTO_TEST_CASE(WindowMode_RequestOutsideBuffer)
{
	...
	TRACE_REQUIRE_EQUAL(fps.size(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 6);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 1);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	auto cf = artdaq::ContainerFragment(*fps.front().first);
	...
	TRACE_REQUIRE_EQUAL(fps.size(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 9);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 2);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	auto cf2 = artdaq::ContainerFragment(*fps.front().first);
...
BOOST_AUTO_TEST_CASE(WindowMode_RequestAfterBuffer)
{
	...
	TRACE_REQUIRE_EQUAL(fps.size(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 11);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 1);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	auto cf = artdaq::ContainerFragment(*fps.front().first);
	...
	TRACE_REQUIRE_EQUAL(fps.size(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->fragmentID(), 1);
	TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 16);
	TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 2);
	type = artdaq::Fragment::ContainerFragmentType;
	TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
	auto cf4 = artdaq::ContainerFragment(*fps.front().first);
...
BOOST_AUTO_TEST_CASE(CircularBufferMode_MultipleIDs)
{
	...
	{
		ids[fps.front().first->fragmentID()]++;
		TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 10);
		TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 6);
		type = artdaq::Fragment::ContainerFragmentType;
		TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
		auto cf = artdaq::ContainerFragment(*fps.front().first);
...
BOOST_AUTO_TEST_CASE(WindowMode_Function_MultipleIDs)
{
	...
	{
		ids[fps.front().first->fragmentID()]++;
		TRACE_REQUIRE_EQUAL(fps.front().first->timestamp(), 15);
		TRACE_REQUIRE_EQUAL(fps.front().first->sequenceID(), 8);
		type = artdaq::Fragment::ContainerFragmentType;
		TRACE_REQUIRE_EQUAL(fps.front().first->type(), type);
		auto cf = artdaq::ContainerFragment(*fps.front().first);
		TRACE_REQUIRE_EQUAL(cf.block_count(), 1);
