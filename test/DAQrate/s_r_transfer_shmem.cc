...
size_t do_sending(fhicl::ParameterSet ps, int sends_each_sender)
{
	...
	for (int ii = 0; ii < sends_each_sender; ++ii)
	{
		...
		*++it = sndDatSz;
		...
		std::cout << "Sender " << my_rank << " sent fragment " << ii << std::endl;
	...
	return tot_wrds * sizeof(artdaq::RawDataType);
} // do_sending

size_t do_receiving(fhicl::ParameterSet ps, int totalReceives)
{
	...
	while (counter > 0) {
		...
		if (senderSlot != artdaq::TransferInterface::RECV_TIMEOUT) {
			if (ignoreFragPtr->type() == artdaq::Fragment::EndOfDataFragmentType) {
				...
				counter--;
				std::cout << "Receiver " << my_rank << " received fragment " << totalReceives - counter << " with seqID " << ignoreFragPtr->sequenceID() << " from Sender " << senderSlot << std::endl;
...
int main(int argc, char * argv[])
{
	...
	for (int ii = 0; ii < num_sending_ranks; ++ii) {
		ss << "s" << ii << ": { transferPluginType: Shmem source_rank: " << ii << " max_fragment_size_words: " << MAX_PAYLOAD_SIZE << " buffer_count: " << BUFFER_COUNT << "}";
	}
	ss << "} destinations: {";
	for (int jj = num_sending_ranks; jj < total_ranks; ++jj) {
		ss << "d" << jj << ": { transferPluginType: Shmem destination_rank: " << jj << " max_fragment_size_words: " << MAX_PAYLOAD_SIZE << " buffer_count: " << BUFFER_COUNT << "}";
	}
	ss << "}";
	...
	if (my_rank < num_sending_ranks) {
		size = do_sending(ps, sends_each_sender);
	}
	else {
		size = do_receiving(ps, totalSends);
	}
	auto duration = std::chrono::duration_cast<std::chrono::duration<double, std::ratio<1>>>(std::chrono::steady_clock::now() - start_time).count();
	std::cout << (my_rank < num_sending_ranks ? "Sent " : "Received ") << size << " bytes in " << duration << " seconds ( " << formatBytes(size / duration) << "/s )." << std::endl;
