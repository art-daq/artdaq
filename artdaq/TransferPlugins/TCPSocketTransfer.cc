...
artdaq::TCPSocketTransfer::
    ...
    , send_ack_diff_(0)
{
	TLOG(TLVL_DEBUG + 32) << GetTraceName() << " Constructor: pset=" << pset.to_string() << ", role=" << (role == TransferInterface::Role::kReceive ? "kReceive" : "kSend");
	connection_was_lost_ = false;

	if (role == TransferInterface::Role::kReceive)
	{
		// Wait for sender to connect...
		TLOG(TLVL_DEBUG + 32) << GetTraceName() << "Listening for connections";
		start_listen_thread_();
		TLOG(TLVL_DEBUG + 32) << GetTraceName() << "Done Listening";
	}
	else
	{
		hostMap_ = MakeHostMap(pset);
		TLOG(TLVL_DEBUG + 32) << GetTraceName() << "Connecting to destination";
		connect_();
		TLOG(TLVL_DEBUG + 32) << GetTraceName() << "Done Connecting";
	}
	TLOG(TLVL_DEBUG + 32) << GetTraceName() << "End of Constructor";
}

artdaq::TCPSocketTransfer::~TCPSocketTransfer() noexcept
{
	TLOG(TLVL_DEBUG + 32) << GetTraceName() << "Shutting down TCPSocketTransfer";
	...
	TLOG(TLVL_DEBUG + 32) << GetTraceName() << "End of Destructor";
}

int artdaq::TCPSocketTransfer::receiveFragmentHeader(detail::RawFragmentHeader& header, size_t timeout_usec)
{
	TLOG(TLVL_DEBUG + 34) << GetTraceName() << "receiveFragmentHeader BEGIN";
	...
	{  // what if just listen_fd???
		...
		TLOG(TLVL_DEBUG + 36) << GetTraceName() << "Receive socket not connected, returning RECV_TIMEOUT";
	...
	TLOG(TLVL_DEBUG + 34) << GetTraceName() << "timeout_usec=" << timeout_usec;
	...
	//{
	//	TLOG(TLVL_DEBUG + 33) << GetTraceName() << "Currently receiving from fd " << active_receive_fd_ << ", waiting!";
	//	usleep(1000);
	// }
	...
	while (!done && getConnectedFDCount_(source_rank()) > 0)
	{
		if (getActiveFD_(source_rank()) == -1)
		{
			...
			// TLOG(TLVL_DEBUG + 32) << GetTraceName() << "receiveFragment: Polling fd to see if there's data" ;
			int num_fds_ready = poll(&pollfds[0], fd_count, timeout_ms);
			if (num_fds_ready <= 0)
			{
				TLOG(TLVL_DEBUG + 34) << GetTraceName() << "No data on receive socket, returning RECV_TIMEOUT";
				return RECV_TIMEOUT;
			}
			...
			for (size_t ii = index; ii < index + pollfds.size(); ++ii)
			{
				...
				{
					TLOG(TLVL_DEBUG + 32) << GetTraceName() << "FD is closed, most likely because the peer went away. Removing from fd list.";
			...
			if (active_index == -1)
			{
				...
				{
					TLOG(TLVL_DEBUG + 32) << GetTraceName() << "Wrong event received from a pollfd. Mask: " << static_cast<int>(anomolous_events);
				}
		...
		if (++loop_guard > 10010)
		{
			TLOG(TLVL_WARNING) << GetTraceName() << "loop guard triggered, returning RECV_TIMEOUT";
			usleep(receive_err_wait_us_);
			setActiveFD_(source_rank(), -1);
			return RECV_TIMEOUT;
		}
		...
		if (byte_cnt > 0)
		{
			TLOG(TLVL_DEBUG + 35) << GetTraceName() << "Reading " << byte_cnt << " bytes from socket " << fd;
			sts = read(fd, buff, byte_cnt);
			TLOG(TLVL_DEBUG + 35) << GetTraceName() << "Done with read sts=" << sts;
		}
		if (sts > 0)
		{
			loop_guard = 0;
		...
		TLOG(TLVL_DEBUG + 36) << GetTraceName() << "state=" << static_cast<int>(state) << " read=" << sts;
		if (sts < 0 && errno != EAGAIN)
		{
			TLOG(TLVL_WARNING) << GetTraceName() << "Error on receive, closing socket " << fd
		...
		else if (sts <= 0 && errno == EAGAIN)
		{
			...
			{
				TLOG(TLVL_WARNING) << GetTraceName() << "No data received, is the sender still sending?!?";
				noDataWarningSent = true;
			}
			if (TimeUtils::GetElapsedTime(last_recv_time_) > receive_disconnected_wait_s_)
			{
				TLOG(TLVL_ERROR) << GetTraceName() << "No data received within timeout, aborting!";
				return RECV_TIMEOUT;
			}
		}
		else
		{
			...
			TLOG(TLVL_DEBUG + 36) << GetTraceName() << "Checking for complete sts=" << sts << ", offset=" << offset << ", target_bytes=" << target_bytes;
			sts = offset += sts;
			if (sts >= target_bytes)
			{
				TLOG(TLVL_DEBUG + 36) << GetTraceName() << "Target read bytes reached. Changing state";
				offset = 0;
				if (state == SocketState::Metadata)
				{
					...
					TLOG(TLVL_DEBUG + 36) << GetTraceName() << "Expected header size = " << target_bytes << ", sizeof(RawFragmentHeader) = " << sizeof(artdaq::detail::RawFragmentHeader);
					...
					{
						TLOG(TLVL_WARNING) << GetTraceName() << "Message header indicates that Fragment data follows when I was expecting a Fragment header!";
				...
				else
				{
					...
					TLOG(TLVL_DEBUG + 35) << GetTraceName() << "done sts=" << sts << " src=" << ret_rank;
					TLOG(TLVL_DEBUG + 36) << GetTraceName() << "Done receiving fragment header. Moving into output.";
	...
	TLOG(TLVL_DEBUG + 34) << GetTraceName() << "Returning " << ret_rank;
	return ret_rank;
}
...
{
	TLOG(TLVL_DEBUG + 39) << GetTraceName() << "receiveFragmentData: BEGIN";
	int ret_rank = RECV_TIMEOUT;
	if (getActiveFD_(source_rank()) == -1)
	{  // what if just listen_fd???
	...
	while (!done)
	{
		...
		if (state == SocketState::Metadata)
		{
			// TLOG(TLVL_DEBUG + 32) << GetTraceName() << "receiveFragmentData: Reading Message Header" ;
			buff = &(mha[offset]);  // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
			byte_cnt = sizeof(MessHead) - offset;
		}
		else
		{
			// TLOG(TLVL_DEBUG + 32) << GetTraceName() << "receiveFragmentData: Reading data" ;
		...
		TLOG(TLVL_DEBUG + 38) << GetTraceName() << "receiveFragmentData: Reading " << byte_cnt << " bytes from socket into " << static_cast<void*>(buff);
		sts = read(getActiveFD_(source_rank()), buff, byte_cnt);
		// TLOG(TLVL_DEBUG + 32) << GetTraceName() << "receiveFragmentData: Done with read" ;

		TLOG(TLVL_DEBUG + 38) << GetTraceName() << "recvFragment state=" << static_cast<int>(state) << " read=" << sts;
...
bool artdaq::TCPSocketTransfer::isRunning()
{
	switch (role())
	{
		...
		case TransferInterface::Role::kReceive:
			...
			TLOG(TLVL_DEBUG + 32) << GetTraceName() << "isRunning: There are " << count << " fds connected.";
			return count > 0;
	}
	return false;
}
...
{
	TLOG(TLVL_DEBUG + 42) << GetTraceName() << "sendFragment begin send of fragment with sequenceID=" << frag.sequenceID();
	...
	{
		TLOG(TLVL_DEBUG + 43) << GetTraceName() << "sendFragment: Timeout sending fragment";
	...
	TLOG(TLVL_DEBUG + 42) << GetTraceName() << "sendFragment returning " << CopyStatusToString(sts);
	return sts;
}
...
{
	TLOG(TLVL_DEBUG + 32) << GetTraceName() << "sendData_ Converting buf to iovec";
...
artdaq::TransferInterface::CopyStatus artdaq::TCPSocketTransfer::sendData_(const struct iovec* iov, int iovcnt, size_t send_timeout_usec, bool isHeader)
{
	...
	if (send_fd_ == -1)
	{
		...
		{
			TLOG(TLVL_DEBUG + 32) << GetTraceName() << "sendData_: Send fd is not open. Returning kTimeout";
			timeoutMessageArmed_ = false;
		}
		return CopyStatus::kTimeout;
	}
	timeoutMessageArmed_ = true;
	TLOG(TLVL_DEBUG + 44) << GetTraceName() << "send_timeout_usec is " << send_timeout_usec << ", currently unused.";

	// TLOG(TLVL_DEBUG + 32) << GetTraceName() << "sendData_: Determining write size" ;
	...
	// TLOG(TLVL_DEBUG + 32) << GetTraceName() << "sendData_: Constructing Message Header" ;
...
// TLOG(TLVL_DEBUG + 32) << GetTraceName() << " calling writev" ;
...
#endif
		// TLOG(TLVL_DEBUG + 32) << GetTraceName() << " done with writev" ;

		if (sts == -1)
		{
			if (errno == EAGAIN /* same as EWOULDBLOCK */)
			{
				TLOG(TLVL_DEBUG + 32) << GetTraceName() << "sendFragment EWOULDBLOCK";
				blocking = 1u;
		...
		if (sts != this_write_bytes)
		{
			...
			TLOG(TLVL_DEBUG + 32) << GetTraceName() << "sendFragment writev sts(" << sts << ")!=requested_send_bytes(" << this_write_bytes << ")";
		...
		{
#ifndef __OPTIMIZE__  // This can be an expensive TRACE call (even if disabled) due to multiplicity of calls
			TLOG(TLVL_DEBUG + 33) << GetTraceName() << "sendFragment writev sts(" << sts << ")==requested_send_bytes(" << this_write_bytes << ")";
#endif
	...
	TLOG(TLVL_DEBUG + 44) << GetTraceName() << "sendFragment sts=" << sts;
...
void artdaq::TCPSocketTransfer::connect_()
{
	...
	{
		TLOG(TLVL_DEBUG + 32) << GetTraceName() << "Connecting sender socket";
	...
	{
		// write connect msg
		TLOG(TLVL_DEBUG + 32) << GetTraceName() << "connect_: Writing connect message";
...
void artdaq::TCPSocketTransfer::reconnect_()
{
	...
	{
		TLOG(TLVL_DEBUG + 33) << GetTraceName() << "check/reconnect";
		return connect_();
	}
}
...
void artdaq::TCPSocketTransfer::receive_acks_()
{
	while (send_fd_ >= 0)
	{
		...
		TLOG(TLVL_DEBUG + 48) << GetTraceName() << "receive_acks_: Polling fd to see if there's data";
		int num_fds_ready = poll(&pollfd_s, 1, 1000);
		if (num_fds_ready <= 0)
		{
			if (num_fds_ready == 0)
			{
				TLOG(TLVL_DEBUG + 48) << GetTraceName() << "receive_acks_: No data on receive socket";
				continue;
			}
		...
		{
			TLOG(TLVL_DEBUG + 32) << GetTraceName() << "receive_acks_: Wrong event received from pollfd: " << pollfd_s.revents;
			break;
		}
		...
		TLOG(TLVL_DEBUG + 47) << GetTraceName() << "receive_acks_: Received ack message, diff is now " << (send_ack_diff_.load() - 1);
		send_ack_diff_--;
	}
}
...
void artdaq::TCPSocketTransfer::listen_(int port, size_t rcvbuf)
{
	...
	{
		TLOG(TLVL_DEBUG + 33) << "listen_: Listening/accepting new connections on port " << port;
		if (listen_fd == -1)
		{
		...
		if (res > 0)
		{
			...
			{
				TLOG(TLVL_DEBUG + 32) << "listen_: Wrong magic bytes in header!";
				close(fd);
				continue;
			}
		...
		{
			TLOG(TLVL_DEBUG + 46) << "listen_: No connections in timeout interval!";
		}
	}
...
size_t artdaq::TCPSocketTransfer::getConnectedFDCount_(int source_rank)
{
	std::lock_guard<std::mutex> lk(fd_mutex_);
#ifndef __OPTIMIZE__
	TLOG(TLVL_DEBUG + 45) << GetTraceName() << "getConnectedFDCount_: count is " << (connected_fds_.count(source_rank) != 0u ? connected_fds_[source_rank].size() : 0);
#endif
...
int artdaq::TCPSocketTransfer::getActiveFD_(int source_rank)
{
	std::lock_guard<std::mutex> lk(fd_mutex_);
#ifndef __OPTIMIZE__
	TLOG(TLVL_DEBUG + 45) << GetTraceName() << "getActiveFD_: fd is " << (active_receive_fds_.count(source_rank) != 0u ? active_receive_fds_[source_rank] : -1);
#endif
...
void artdaq::TCPSocketTransfer::setActiveFD_(int source_rank, int fd)
{
	std::lock_guard<std::mutex> lk(fd_mutex_);
#ifndef __OPTIMIZE__
	TLOG(TLVL_DEBUG + 45) << GetTraceName() << "setActiveFD_: setting active fd for rank " << source_rank << " to " << fd;
#endif
...
int artdaq::TCPSocketTransfer::getLastActiveFD_(int source_rank)
{
	std::lock_guard<std::mutex> lk(fd_mutex_);
#ifndef __OPTIMIZE__
	TLOG(TLVL_DEBUG + 45) << GetTraceName() << "getLastActiveFD_: fd is " << (last_active_receive_fds_.count(source_rank) != 0u ? last_active_receive_fds_[source_rank] : -1);
#endif
...
void artdaq::TCPSocketTransfer::setLastActiveFD_(int source_rank, int fd)
{
	std::lock_guard<std::mutex> lk(fd_mutex_);
#ifndef __OPTIMIZE__
	TLOG(TLVL_DEBUG + 45) << GetTraceName() << "setLastActiveFD_: setting last active fd for rank " << source_rank << " to " << fd;
#endif
