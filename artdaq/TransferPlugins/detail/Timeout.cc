...
Timeout::get_next_timeout_msdly()
{
    ...
    else {   // NOTE THE + 1 b/c of integer division and/or system HZ resolution
		tmo = delay_us / 1000;
    }
    return (tmo);
    ...
    if (active_time_.size() == 0) {
		...
		if (tmospecs_[idx].tmo_tod_us < tod_now_us) {
			...
			if (tmo.period_us) {
				...
				while (1) {  // see also in cancel_timeout below
					if (i2->second == idx)
						break;
					++i2;
				}
			  ...
			  , std::string	desc )
{
	...
	ii = pairOfIters.first; ee=pairOfIters.second;
	for (; ii!=ee && ii->first.compare(desc)==0; ++ii) {
		...
		if (tmospecs_[idx].tag == tag) {
			...
			while (1) {  // see also in get_clear_next_expired_timeout above
				if (i2->second == idx)
					break;
				++i2;
			}
