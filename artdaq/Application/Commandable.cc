...
{
	TLOG(TLVL_DEBUG) << "do_initialize called." << TLOG_ENDL;
	external_request_status_ = true;
	return external_request_status_;
}
...
{
	TLOG(TLVL_DEBUG) << "do_start called." << TLOG_ENDL;
	external_request_status_ = true;
	return external_request_status_;
}
...
{
	TLOG(TLVL_DEBUG) << "do_stop called." << TLOG_ENDL;
	external_request_status_ = true;
	return external_request_status_;
}
...
{
	TLOG(TLVL_DEBUG) << "do_pause called." << TLOG_ENDL;
	external_request_status_ = true;
	return external_request_status_;
}
...
{
	TLOG(TLVL_DEBUG) << "do_resume called." << TLOG_ENDL;
	external_request_status_ = true;
	return external_request_status_;
}
...
{
	TLOG(TLVL_DEBUG) << "do_shutdown called." << TLOG_ENDL;
	external_request_status_ = true;
	return external_request_status_;
}
...
{
	TLOG(TLVL_DEBUG) << "do_reinitialize called." << TLOG_ENDL;
	external_request_status_ = true;
	return external_request_status_;
}
...
{
	TLOG(TLVL_DEBUG) << "do_soft_initialize called." << TLOG_ENDL;
	external_request_status_ = true;
	return external_request_status_;
}
...
{
	TLOG(TLVL_DEBUG) << "BootedEnter called." << TLOG_ENDL;
}

void artdaq::Commandable::InRunExit()
{
	TLOG(TLVL_DEBUG) << "InRunExit called." << TLOG_ENDL;
}


std::string artdaq::Commandable::do_trace_get(std::string const& name)
{
	TLOG(TLVL_DEBUG) << "Getting masks for name " << name << TLOG_ENDL;
...
{
	TLOG(TLVL_DEBUG) << "Setting msk " << type << " for name " << name << " to " << std::hex << std::showbase << mask << TLOG_ENDL;
	if (name != "ALL")
	{
		if (type.size() > 0) {
			switch (type[0])
...
{
	TLOG(TLVL_DEBUG) << "Meta-Command called: cmd=" << cmd << ", arg=" << arg << TLOG_ENDL;
	return true;
}
