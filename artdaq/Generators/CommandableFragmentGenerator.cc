...
void artdaq::CommandableFragmentGenerator::applyRequestsBufferMode(artdaq::PostmarkedFragmentPtrs& frags)
{
	...
	for (auto& id : dataBuffers_)
	{
		...
		std::pair<artdaq::FragmentPtr, int> pm_frag = std::make_pair(std::move(newfrag), artdaq::Fragment::InvalidDestinationRank);
		frags.emplace_back(std::move(pm_frag));
		frags.back().first->setTimestamp(requests[ev_counter()]);
...
void artdaq::CommandableFragmentGenerator::applyRequestsSequenceIDMode(artdaq::PostmarkedFragmentPtrs& frags)
{
...
void artdaq::CommandableFragmentGenerator::sendEmptyFragments(artdaq::PostmarkedFragmentPtrs& frags, std::map<Fragment::sequence_id_t, Fragment::timestamp_t>& requests)
{
	if (requests.size() > 0)
	{
