    ...
    , sleep_on_stop_us_(0)
{
	...
	if (mode_ != RequestMode::Ignored)
	{
		...
		requestReceiver_ = std::make_unique<RequestReceiver>(ps);
	}
}
...
bool artdaq::CommandableFragmentGenerator::getNext(FragmentPtrs& output)
{
	...
	if (check_stop())
	{
		usleep(sleep_on_stop_us_);
	}
	if (exception() || force_stop_)
	{
		return false;
	}
...
bool artdaq::CommandableFragmentGenerator::check_stop()
{
	...
	if (!useDataThread_ || mode_ == RequestMode::Ignored)
	{
		return true;
	}
	if (force_stop_)
...
void artdaq::CommandableFragmentGenerator::getDataLoop()
{
	...
	while (!force_stop_)
	{
		...
		if (newData.empty() && sleep_on_no_data_us_ > 0)
		{
		...
		while (dataIter != newData.end())
		{
			...
			if (dataBuffers_.count(frag_id) == 0u)
			{
			...
			if (!waitForDataBufferReady(frag_id))
			{
				return;
			}
		...
		if (!data || force_stop_)
		{
			...
			if (requestReceiver_)
			{
				requestReceiver_->ClearRequests();
			}
			newData.clear();
...
bool artdaq::CommandableFragmentGenerator::waitForDataBufferReady(Fragment::fragment_id_t id)
{
	...
	while (dataBufferIsTooLarge(id))
	{
		if (!circularDataBufferMode_)
		{
			...
			if (((waittime % 5) != 0u) && waittime != lastwaittime)
			{
...
bool artdaq::CommandableFragmentGenerator::dataBufferIsTooLarge(Fragment::fragment_id_t id)
{
	if (dataBuffers_.count(id) == 0u)
	{
		...
		throw cet::exception("DataBufferError") << "Error in CommandableFragmentGenerator: Cannot check size of data buffer for ID " << id << " because it does not exist!";  // NOLINT(cert-err60-cpp)
	}
...
void artdaq::CommandableFragmentGenerator::getDataBufferStats(Fragment::fragment_id_t id)
{
	if (dataBuffers_.count(id) == 0u)
	{
		...
		throw cet::exception("DataBufferError") << "Error in CommandableFragmentGenerator: Cannot get stats of data buffer for ID " << id << " because it does not exist!";  // NOLINT(cert-err60-cpp)
	}
...
void artdaq::CommandableFragmentGenerator::checkDataBuffer(Fragment::fragment_id_t id)
{
	if (dataBuffers_.count(id) == 0u)
	{
		...
		throw cet::exception("DataBufferError") << "Error in CommandableFragmentGenerator: Cannot check data buffer for ID " << id << " because it does not exist!";  // NOLINT(cert-err60-cpp)
	}
	...
	if (dataBuffers_[id].DataBufferDepthFragments > 0 && mode_ != RequestMode::Single && mode_ != RequestMode::Ignored)
	{
		...
		if (!dataBuffers_[id].DataBuffer.empty())
		{
...
void artdaq::CommandableFragmentGenerator::applyRequestsSingleMode(artdaq::FragmentPtrs& frags)
{
	...
	if (requests.empty() || (requests.count(ev_counter()) == 0u))
	{
		return;
	}
	...
	for (auto& id : dataBuffers_)
	{
		if (!id.second.DataBuffer.empty())
		{
			...
			for (auto& fragptr : id.second.DataBuffer)
			{
				...
				auto newfrag = std::make_unique<artdaq::Fragment>(ev_counter(), frag->fragmentID());
...
void artdaq::CommandableFragmentGenerator::applyRequestsWindowMode_CheckAndFillDataBuffer(artdaq::FragmentPtrs& frags, artdaq::Fragment::fragment_id_t id, artdaq::Fragment::sequence_id_t seq, artdaq::Fragment::timestamp_t ts)
{
	...
	TLOG(TLVL_APPLYREQUESTS) << "ApplyRequestsWindowsMode_CheckAndFillDataBuffer: min is " << min << ", max is " << max
	                         << " and first/last points in buffer are " << (!dataBuffers_[id].DataBuffer.empty() ? dataBuffers_[id].DataBuffer.front()->timestamp() : 0)
	                         << "/" << (!dataBuffers_[id].DataBuffer.empty() ? dataBuffers_[id].DataBuffer.back()->timestamp() : 0)
	                         << " (sz=" << dataBuffers_[id].DataBuffer.size() << " [" << dataBuffers_[id].DataBufferDepthBytes.load()
	                         << "/" << maxDataBufferDepthBytes_ << "])";
	bool windowClosed = !dataBuffers_[id].DataBuffer.empty() && dataBuffers_[id].DataBuffer.back()->timestamp() >= max;
	...
	if (windowTimeout)
	{
		TLOG(TLVL_WARNING) << "applyRequestsWindowMode_CheckAndFillDataBuffer: A timeout occurred waiting for data to close the request window ({" << min << "-" << max
		                   << "}, buffer={" << (!dataBuffers_[id].DataBuffer.empty() ? dataBuffers_[id].DataBuffer.front()->timestamp() : 0) << "-"
		                   << (!dataBuffers_[id].DataBuffer.empty() ? dataBuffers_[id].DataBuffer.back()->timestamp() : 0)
		                   << "} ). Time waiting: "
	...
	if (windowClosed || !data_thread_running_ || windowTimeout)
	{
		...
		if (!windowClosed || (!dataBuffers_[id].DataBuffer.empty() && dataBuffers_[id].DataBuffer.front()->timestamp() > min))
		{
		...
		if (seq > dataBuffers_[id].HighestRequestSeen)
		{
			dataBuffers_[id].HighestRequestSeen = seq;
		}
	}
}
...
void artdaq::CommandableFragmentGenerator::applyRequestsWindowMode(artdaq::FragmentPtrs& frags)
{
	...
	for (auto req = requests.begin(); req != requests.end();)
	{
		...
		while (req->first < ev_counter() && !requests.empty())
		{
		...
		for (auto& id : dataBuffers_)
		{
			if (id.second.WindowsSent.count(req->first) == 0u)
			{
...
void artdaq::CommandableFragmentGenerator::applyRequestsSequenceIDMode(artdaq::FragmentPtrs& frags)
{
	...
	for (auto req = requests.begin(); req != requests.end();)
	{
		...
		for (auto& id : dataBuffers_)
		{
			...
			if (req->first > id.second.HighestRequestSeen)
			{
...
bool artdaq::CommandableFragmentGenerator::applyRequests(artdaq::FragmentPtrs& frags)
{
	...
	if (mode_ == RequestMode::Ignored)
	{
		...
		{
			if (check_stop() || exception() || !isHardwareOK_)
			{
				return false;
			}
			std::unique_lock<std::mutex> lock(dataBuffersMutex_);
	...
	{
		if ((check_stop() && requestReceiver_->size() == 0) || exception())
		{
			return false;
		}
...
bool artdaq::CommandableFragmentGenerator::sendEmptyFragment(artdaq::FragmentPtrs& frags, size_t seqId, Fragment::fragment_id_t fragmentId, const std::string& desc)
{
...
void artdaq::CommandableFragmentGenerator::checkSentWindows(artdaq::Fragment::sequence_id_t seq)
{
	...
	for (auto& id : dataBuffers_)
	{
		if ((id.second.WindowsSent.count(seq) == 0u) || id.second.HighestRequestSeen < seq)
		{
			seqComplete = false;
		}
		if ((id.second.WindowsSent.count(seq) != 0u) && TimeUtils::GetElapsedTimeMicroseconds(id.second.WindowsSent[seq]) > missing_request_window_timeout_us_)
		{
			seqTimeout = true;
		}
	}
	...
	if (seqTimeout)
	{
		...
		while (ev_counter() <= seq)
		{
			if (ev_counter() < seq)
			{
				TLOG(TLVL_WARNING) << "Missed request for sequence ID " << ev_counter() << "! Will not send any data for this sequence ID!";
			}
			requestReceiver_->RemoveRequest(ev_counter());
