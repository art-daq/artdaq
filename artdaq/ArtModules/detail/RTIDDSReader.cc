...
artdaq::detail::RTIDDSReader::RTIDDSReader(fhicl::ParameterSet const & ps,
...
void artdaq::detail::RTIDDSReader::closeCurrentFile()
{
}

void artdaq::detail::RTIDDSReader::readFile(string const &,
    art::FileBlock *& fb)
{
  fb = new art::FileBlock(art::FileFormatVersion(1, "RawEvent2011"), "nothing");
}

bool artdaq::detail::RTIDDSReader::readNext(art::RunPrincipal * const & inR,
  ...
  while (keep_looping) {
    ...
    if (!got_event) {
      mf::LogInfo("InputFailure")
          << "Reading timed out in RTIDDSReader::readNext()";
