...
#define TRACE_NAME "ListenTransferWrapper"
...
artdaq::FragmentPtrs artdaq::ListenTransferWrapper::receiveMessage()
{
	...
	while (gSignalStatus == 0)
	{
		...
		{
			throw cet::exception("ListenTransferWrapper") << "First " << maxEventsBeforeInit_ << " events received did not include the \"Init\" event containing necessary info for art; exiting...";  // NOLINT(cert-err60-cpp)
		}
	}
...
void artdaq::ListenTransferWrapper::checkIntegrity(
	...
	if (totalsize != artdaqheader + metadata + payload)
	{
		...
		{
			throw cet::exception("ListenTransferWrapper") << errmsg.str();  // NOLINT(cert-err60-cpp)
		}
...
artdaq::ListenTransferWrapper::~ListenTransferWrapper()
{
