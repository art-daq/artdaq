...
{
	TLOG(10) << "RootDAQOut::write begin - line " << __LINE__;
	if (dropAllEvents_) {
		return;
	}
	...
	TLOG(10) << "RootDAQOut::write done/return";
}
...
isFileOpen() const
{
	TLOG(10) << "RootDAQOut::isFileOpen start";
	bool ret = rootOutputFile_.get() != nullptr;
	TLOG(10) << "RootDAQOut::isFileOpen return " << ret;
	return ret;
}
...
{
	TLOG(10) << "RootDAQOut::fileSwitchBoundary start";
	auto bb = fileProperties_.granularity();
	TLOG(10) << "RootDAQOut::fileSwitchBoundary done/return";
	return bb;
}
#endif
