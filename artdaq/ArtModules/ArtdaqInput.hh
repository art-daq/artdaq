...
art::ArtdaqInput<U>::ArtdaqInput(const fhicl::ParameterSet& ps, art::ProductRegistryHelper& helper,
	...
	: shutdownMsgReceived_(false), outputFileCloseNeeded_(false), pm_(pm), communicationWrapper_(ps) {
...
#else
  auto productlist = std::unique_ptr<art::ProductList>(ReadObjectAny<art::ProductList>(
	  msg, "std::map<art::BranchKey,art::BranchDescription>", "ArtdaqInput::ArtdaqInput"));
  helper.productList(std::move(productlist));
#endif
...
void art::ArtdaqInput<U>::readAndConstructPrincipal(std::unique_ptr<TBufferFile>& msg, unsigned long msg_type_code,
  ...
  if (msg_type_code == 2) {  // EndRun message.
	...
	if ((inSR == nullptr) || !inSR->SUBRUN_ID().isValid() || (inSR->subRun() != event_aux->subRun())) {
...
bool art::ArtdaqInput<U>::readNext(art::RunPrincipal* const inR, art::SubRunPrincipal* const inSR,
  ...
  if (msg_type_code == 2) {
	...
	if (inR != 0 && inSR != 0 && outR != 0 && outSR != 0) {
	  if (inR->RUN_ID().isFlush() && inSR->SUBRUN_ID().isFlush() && outR->RUN_ID().isFlush() && outSR->SUBRUN_ID().isFlush()) {
				outR = 0;
				outSR = 0;
