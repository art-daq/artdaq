...
art::RootNetOutput::RootNetOutput(fhicl::ParameterSet const& ps) : OutputModule(ps), initMsgSent_(false) {
...
static void send_init_message() {
  ...
  if (product_list_class == nullptr) {
    throw art::Exception(art::errors::DictionaryNotFound) << "RootNetOutput static send_init_message(): "
  ...
  //
  TLOG(TLVL_SENDINIT) << "static send_init_message(): Streaming ParentageRegistry ..." << (void*)parentage_map_class;
  ...
  TLOG(TLVL_SENDINIT) << "RootNetOutput static send_init_message(): Sending the init message to "
