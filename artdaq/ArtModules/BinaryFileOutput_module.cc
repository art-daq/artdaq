...
art::BinaryFileOutput::BinaryFileOutput(ParameterSet const& ps) : OutputModule(ps), fstats_{name_, processName()} {
...
void art::BinaryFileOutput::deinitialize_FILE_() {
  if (do_direct_) {
		close(fd_);
		fd_ = -1;
  } else
...
bool art::BinaryFileOutput::readParameterSet_(fhicl::ParameterSet const& pset) {
  ...
  try {
    ...
    TLOG(TLVL_ERROR) << name_ << "The fileName parameter was not specified "
...
void art::BinaryFileOutput::write(EventPrincipal& ep) {
