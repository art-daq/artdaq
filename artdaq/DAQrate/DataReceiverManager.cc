...
void artdaq::DataReceiverManager::runReceiver_(int source_rank)
{
	...
	{
		TLOG(16) << "DataReceiverManager::runReceiver_: Begin loop";
		...
		TLOG(16) << "DataReceiverManager::runReceiver_: Calling receiveFragmentHeader";
		ret = source_plugins_[source_rank]->receiveFragmentHeader(header, receive_timeout_);
		TLOG(16) << "DataReceiverManager::runReceiver_: Done with receiveFragmentHeader, ret="<< ret <<" (should be "<< source_rank <<")";
		if (ret != source_rank)
		{
		...
		if (Fragment::isUserFragmentType(header.type) || header.type == Fragment::DataFragmentType || header.type == Fragment::EmptyFragmentType || header.type == Fragment::ContainerFragmentType) {
			...
			TLOG(16) << "DataReceiverManager::runReceiver_: Calling receiveFragmentData";
			auto ret2 = source_plugins_[source_rank]->receiveFragmentData(loc, header.word_count - header.num_words());
			TLOG(16) << "DataReceiverManager::runReceiver_: Done with receiveFragmentData, ret2="<< ret2 <<" (should be "<< source_rank <<")";
			...
			{//&& recv_frag_count_.slotCount(source_rank) % 100 == 0) {
				TLOG(6) << "DataReceiverManager::runReceiver_: Sending receive stats";
				...
				TLOG(6) << "DataReceiverManager::runReceiver_: Done sending receive stats";
			}
		}
