...
sendFragment(Fragment&& frag)
{
	...
	else if (non_blocking_mode_)
	{
		...
		while (dest == TransferInterface::RECV_TIMEOUT && count > 0) {
			...
			{
				count--;
				TLOG_WARNING("DataSenderManager") << "Could not get destination for seqID " << std::to_string(seqID) << (count > 0 ? ", retrying." : ".") << TLOG_ENDL;
			}
		}
	...
	if (metricMan) {//&& sent_frag_count_.slotCount(dest) % 100 == 0) {
		...
		if (use_routing_master_) {
			...
			if (routing_wait_time_ > 0)
				metricMan->sendMetric("Routing Wait Time", static_cast<double>(routing_wait_time_.load()) / 1000000, "s", 1,
					MetricMode::Average);
		}
	}
