    ...
    , highest_sequence_id_routed_(0)
{
	...
	if (use_routing_manager_ && pset.get<bool>("use_routing_table_thread", true))
	{
...
void artdaq::TableReceiver::connectToRoutingManager_()
{
	...
	detail::RoutingConnectHeader startHdr(my_rank);
	write(table_socket_, &startHdr, sizeof(startHdr));
}

void artdaq::TableReceiver::disconnectFromRoutingManager_()
{
	detail::RoutingConnectHeader endHdr(my_rank, detail::RoutingConnectHeader::ConnectHeaderMode::Disconnect);
...
bool artdaq::TableReceiver::receiveTableUpdate_()
{
	...
	stss = recv(table_socket_, &buffer, sizeof(artdaq::detail::RoutingPacketEntry) * hdr.nEntries, MSG_WAITALL);
	if (stss != static_cast<ssize_t>(sizeof(artdaq::detail::RoutingPacketEntry) * hdr.nEntries))
	{
