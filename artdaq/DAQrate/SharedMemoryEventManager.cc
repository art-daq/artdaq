...
void artdaq::SharedMemoryEventManager::RunArt(std::shared_ptr<art_config_file> config_file, pid_t& pid_out)
{
	while (restart_art_)
	{
		...
		{
			TLOG_WARNING("SharedMemoryEventManager") << "art process " << pid << " exited with status code 0x" << std::hex << status << ", " << (restart_art_ ? "restarting" : "not restarting") << TLOG_ENDL;
		}
	}
}
...
{
	{
		...
		for (auto buf : sorted_buffers)
		{
			...
			TLOG_DEBUG("SharedMemoryEventManager") << "Releasing event " << std::to_string(hdr->sequence_id) << " to art." << TLOG_ENDL;
			MarkBufferFull(buf);
			subrun_event_count_++;
