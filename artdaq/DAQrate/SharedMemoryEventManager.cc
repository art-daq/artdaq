                  ...
                  pset.get<int>("expected_art_event_processing_time_us", 100000) * pset.get<size_t>("buffer_count"), false)
{
	...
	{
		auto conf = pset.get<std::vector<Fragment::fragment_id_t>>("expected_fragment_ids");
		for (auto f : conf)
		{
...
bool artdaq::SharedMemoryEventManager::AddFragment(detail::RawFragmentHeader frag, void* dataPtr)
{
	...
	if (buffer == -2)
	{
		TLOG(TLVL_ERROR) << "Dropping fragment because data taking has already passed this event number: " << frag.sequence_id;
		return true;
	}

	{
		...
		if (in_progress_fragment_ids_.count(frag.sequence_id) && in_progress_fragment_ids_[frag.sequence_id].size() > 0)
		{
			...
			{
				TLOG(TLVL_ERROR) << "Dropping fragment because " << frag.fragment_id << " is not in the list of expected Fragment IDs for event " << frag.sequence_id << "!";
				return true;
			}
			if (in_progress_fragment_ids_[frag.sequence_id][frag.fragment_id])
			{
				TLOG(TLVL_ERROR) << "Dropping fragment because " << frag.fragment_id << " has already been added to event " << frag.sequence_id << "!";
				return true;
			}
			in_progress_fragment_ids_[frag.sequence_id][frag.fragment_id] = true;
		}
	}
	...
	TLOG(TLVL_TRACE) << "Checking for complete event";
	{
		...
		if (in_progress_fragment_ids_.count(frag.sequence_id) && in_progress_fragment_ids_[frag.sequence_id].size() > 0)
		{
			...
			hdr->is_complete = count == in_progress_fragment_ids_[frag.sequence_id].size() && buffer_writes_pending_[buffer] == 0;
			TLOG(TLVL_TRACE) << "hdr->is_complete=" << std::boolalpha << hdr->is_complete
			                 << ", fragmentCount=" << count
			                 << ", expected=" << in_progress_fragment_ids_[frag.sequence_id].size()
...
void artdaq::SharedMemoryEventManager::DoneWritingFragment(detail::RawFragmentHeader frag)
{
	...
	statsHelper_.addSample(FRAGMENTS_RECEIVED_STAT_KEY, frag.word_count * sizeof(RawDataType));
	{
		...
		std::unique_lock<std::mutex> buffer_lk(buffer_mutexes_.at(buffer));
		...
		size_t frag_count = 0;
		{
			...
			{
				frag_count = GetFragmentCountInBuffer(buffer);
...
bool artdaq::SharedMemoryEventManager::endOfData()
{
	...
	while (lastReadCount > 0 && (end_of_data_wait_us == 0 || TimeUtils::GetElapsedTimeMicroseconds(start) < end_of_data_wait_us) && get_art_process_count_() > 0)
	{
		...
		if (lastReadCount > 0)
		{
			TLOG(19) << "About to sleep " << outstanding_buffer_wait_time << " us - lastReadCount=" << lastReadCount << " size=" << size() << " end_of_data_wait_us=" << end_of_data_wait_us;
...
void artdaq::SharedMemoryEventManager::startRun(run_id_t runID)
{
	...
	{
		std::lock_guard<std::mutex> fragment_ids_lock(fragment_ids_mutex_);
		fragment_id_overrides_.clear();
		in_progress_fragment_ids_.clear();
	}
	run_event_count_ = 0;
	run_incomplete_event_count_ = 0;
...
void artdaq::SharedMemoryEventManager::sendMetrics()
{
	...
	if (incomplete_event_report_interval_ms_ > 0 && (GetLockedBufferCount() != 0u))
	{
		...
		std::lock_guard<std::mutex> fragment_ids_lock(fragment_ids_mutex_);
		for (auto& ev : in_progress_fragment_ids_)
		{
			oss2 << ev.first << ": ";
			for (auto& fragment_id : ev.second)
			{
				oss2 << "ID " << fragment_id.first << " [" << (fragment_id.second ? "X" : " ") << "], ";
			}
		}
	}
}
...
bool artdaq::SharedMemoryEventManager::broadcastFragments_(FragmentPtrs& frags)
{
	...
	TLOG(TLVL_DEBUG) << "broadcastFragments_: after getting buffer w/timeout, buffer=" << buffer << ", elapsed time=" << TimeUtils::GetElapsedTime(start_time) << " s.";
	if (buffer == -1)
	{
	...
	TLOG(TLVL_DEBUG) << "broadcastFragments_: Filling in RawEventHeader";
	...
	for (auto& frag : frags)
	{
		TLOG(TLVL_DEBUG) << "broadcastFragments_ before Write calls";
...
void artdaq::SharedMemoryEventManager::OverrideFragmentIDsForEvent(Fragment::sequence_id_t seqID, std::set<Fragment::fragment_id_t> frags)
{
	std::lock_guard<std::mutex> fragment_ids_lock(fragment_ids_mutex_);
